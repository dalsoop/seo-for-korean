...
* Test: new Autoload_Test that walks every includes/**/class-*.php and asserts each declared class autoloads — catches FQCN ↔ filesystem mismatches the autoloader silently swallows.
...
* Fix: Article schema no longer emits a dangling 'author' @id when post_author = 0 (e.g., posts created via wp-cli without --user=).
